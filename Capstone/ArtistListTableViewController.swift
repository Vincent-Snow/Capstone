...
class ArtistListTableViewController: UITableViewController {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "toAlbumList" {
            if let destinationViewController = segue.destinationViewController as? AlbumListTableViewController {
                let indexPath = tableView.indexPathForSelectedRow
                
                if let selectedRow = indexPath?.row {
                    let artist = artists[selectedRow]
                    destinationViewController.albums = artist.albums ?? []
                }
